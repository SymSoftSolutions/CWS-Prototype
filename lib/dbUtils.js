 ...
 * @returns {Promise}
 */
function updateUserDetails(user, details){
    function updateDetails(user){
        var oldDetails = user.userDetails;
        var merged = Object.assign({}, oldDetails, details);
        return db.from('users').modify(whereUniqueUser,user).update({
            userDetails : JSON.stringify(merged)
        });
    }
    function getUserDetails(trx){
        return trx.first().modify(whereUniqueUser, user).from('users')
            .then(updateDetails);
    }
    return db.transaction(getUserDetails)
}
