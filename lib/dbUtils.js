 ...
 * @param {Object} details - object with properties which will be merged  
 * @returns {Promise}
 */
exports.updateUserDetails = function updateUserDetails(user, updatedUser){

    function filterNonColumns() {
        return listTableColumns('users').then(function(cols){
            var columnNames = cols.map(function(col) { return col.column_name; });
            // using only the column names create a sanitized user object
            var sanitizedUser = {};
            columnNames.map(function(col){
                sanitizedUser[col] = updatedUser[col]
            })
            console.log("Sanitized", sanitizedUser);
            return Promise.resolve(sanitizedUser);
        })
    }
    function updateDetails(user){
        // merge our old json data first
        updatedUser.userDetails =  Object.assign({},  user.userDetails, updatedUser.userDetails);
        // then update the whole user object
        return db.from('users').modify(whereUniqueUser, user).update(updatedUser);
    }
    return filterNonColumns().then(retrieveUser).then(updateDetails)
}
