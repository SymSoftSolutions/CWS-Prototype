...
function markMessageRead(messageID) {
    db.select().where('messageID', messageID)
               .from('messages')
               .then(function(rows) {
               console.log(rows);
    });
...
function deleteMessage(messageID) {
   return  db.select().where('messageID', messageID)
             .del();
}
 ...
 * @returns {*}
 */
function deserializeUser(req, email, cb) {

    return db.select().where('email', email).from('users').then(function (rows) {
        // no user found
        if (rows.rowCount == 0) {
            return cb(null, false);
        }
